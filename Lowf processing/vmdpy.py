...
def find_peaks(signal, threshold=0.1):
    ...
    # 如果 threshold 为 0，则返回所有极大值点
    if threshold == 0:
        return peaks

    # 过滤掉小于最大极大值点值 threshold 倍的点
    filtered_peaks = peaks[signal[peaks] >= max_peak_value * threshold]
    # 找到最大的前6个极大值点
    top_six_peaks = np.argsort(peak_values)[-8:][::-1]  # 从大到小排序，取前五个
    top_six_indices = peaks[top_six_peaks]

    #return filtered_peaks
    return top_six_indices
